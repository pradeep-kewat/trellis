...
import {
  createMasterTraits,
  fetchMasterTableTraits,
  fetchMasterTraitsCount,
  updateMasterTraits,
...
const MasterTraits = () => {
  ...
  const fetchTableCount = async (payload: any) => {
    return (await fetchMasterTraitsCount(payload)).count || 0;
  };

  const fetchTableData = async (payload: any) => {
    return (await fetchMasterTableTraits(payload)).data || [];
  };

  const handleMenuChange = async (actionId: string, data: any) => {
    const payload = { action: data, actionId };
    const response = await updateMasterTraits(payload);
  ...
  const handleCreate = async (data: any) => {
    const response = await createMasterTraits(data);
    if (response.success) return { shouldRefetch: true };
  };

  const handleUpdate = async (data: any) => {
    const response = await updateMasterTraits(data);
    if (response.success) return { shouldRefetch: true };
  };
