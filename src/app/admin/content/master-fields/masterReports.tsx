...
import {
  createMasterReports,
  fetchMasterReportsCount,
  fetchMasterTableReports,
  updateMasterReports,
...
const headers: Header = {
  name: "Master Personality Traits",
  hasFilters: false,
  properties: [
...
const MasterReports = () => {
  ...
  const fetchTableCount = async (payload: any) => {
    return (await fetchMasterReportsCount(payload)).count || 0;
  };

  const fetchTableData = async (payload: any) => {
    return (await fetchMasterTableReports(payload)).data || [];
  };

  const handleMenuChange = async (actionId: string, data: any) => {
    const payload = { action: data, actionId };
    const response = await updateMasterReports(payload);
  ...
  const handleCreate = async (data: any) => {
    const response = await createMasterReports(data);
    if (response.success) return { shouldRefetch: true };
  };

  const handleUpdate = async (data: any) => {
    const response = await updateMasterReports(data);
    if (response.success) return { shouldRefetch: true };
  };
