...
import {
  updateMasterSkills,
  fetchMasterTableSkills,
  fetchMasterSkillsCount,
  createMasterSkills,
...
const MasterSkills = () => {
  ...
  const fetchTableCount = async (payload: any) => {
    return (await fetchMasterSkillsCount(payload)).count || 0;
  };

  const fetchTableData = async (payload: any) => {
    return (await fetchMasterTableSkills(payload)).data || [];
  };

  const handleMenuChange = async (actionId: string, data: any) => {
    const payload = { action: data, actionId };
    const response = await updateMasterSkills(payload);
  ...
  const handleCreate = async (data: any) => {
    const response = await createMasterSkills(data);
    if (response.success) return { shouldRefetch: true };
  };

  const handleUpdate = async (data: any) => {
    const response = await updateMasterSkills(data);
    if (response.success) return { shouldRefetch: true };
  };
