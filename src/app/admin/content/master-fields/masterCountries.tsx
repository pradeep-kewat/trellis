...
import {
  fetchMasterCountriesCount,
  fetchMasterCountries,
  updateMasterCountries,
  createMasterCountries,
...
const MasterCountries = () => {
  ...
  const fetchTableCount = async (payload: any) => {
    return (await fetchMasterCountriesCount(payload)).count || 0;
  };

  const fetchTableData = async (payload: any) => {
    return (await fetchMasterCountries(payload)).data || [];
  };

  const handleMenuChange = async (actionId: string, data: any) => {
    const payload = { action: data, actionId };
    const response = await updateMasterCountries(payload);
  ...
  const handleCreate = async (data: any) => {
    const response = await createMasterCountries(data);
    if (response.success) return { shouldRefetch: true };
  };

  const handleUpdate = async (data: any) => {
    const response = await updateMasterCountries(data);
    if (response.success) return { shouldRefetch: true };
  };
